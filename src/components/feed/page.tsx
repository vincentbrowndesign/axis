...
function chainTokens(chain: PossessionChain) {
  const tokens: string[] = [];

  chain.links.forEach((link) => {
    if (link.startAction) tokens.push(titleCase(link.startAction));
    if (link.paintTouch === "paint_yes") tokens.push("Paint");
    if (link.help === "help_yes") tokens.push("Help");
    if (link.passTarget) tokens.push(titleCase(link.passTarget));
  });

  if (chain.outcome) tokens.push(titleCase(chain.outcome.replaceAll("_", " ")));
...
export default function Page() {
  ...
  const [activeTab, setActiveTab] = useState<"feed" | "review" | "export">("feed");
  ...
  const reviewedChains = useMemo(
    () =>
      REVIEW_STEPS.map((step) => chains[step.id]).filter(
  ...
  const reviewedPossessions = useMemo(
    () => reviewedChains.map(chainToExportPossession),
    [reviewedChains]
  );
  ...
  const bestThumbnailUrl = useMemo(() => {
    const withThumb = reviewedChains.find((chain) => chain.thumbnailUrl);
  ...
  const feedPossessions: FeedPossession[] = useMemo(() => {
    return reviewedChains.map((chain, index) => {
      ...
      return {
        id: chain.id,
        playerName: "Player",
        sessionLabel: videoName === "No video selected" ? "Session" : videoName,
        state: intelligence.state,
        chain: chainTokens(chain),
        story: chainSummary(chain),
        tags: intelligence.tags.slice(0, 4),
        videoUrl,
        thumbnailUrl: chain.thumbnailUrl,
      };
    });
  }, [reviewedChains, videoName, videoUrl]);
  ...
  }, [videoName, sessionIntel.total, reviewedChains.length, taggedCount, bestThumbnailUrl]);
