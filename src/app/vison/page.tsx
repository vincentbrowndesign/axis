...
function inferSuggestion(samples: Sample[]): PoseSuggestion {
  const last = samples[samples.length - 1];
  const lane = last ? getLane(last.x) : "Middle";
  const avg = averageDelta(samples);

  return {
    lane,
    downhill: avg.dy < -0.0025,
    help: "Unknown",
    confidence: last?.score ?? 0,
  };
}

function getHipCenter(keypoints: poseDetection.Keypoint[]) {
...
export default function VisionPage() {
  ...
  const [skeleton, setSkeleton] = useState<poseDetection.Keypoint[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadModel() {
      ...
      try {
        ...
        if (mounted) setPoseError(message);
      } finally {
                      ...
                      {skeleton.map((kp, index) => {
                        ...
                        return (
                          <circle
                            ...
                            cx={(kp.x / (videoRef.current?.videoWidth || 1)) * 100}
                            cy={(kp.y / (videoRef.current?.videoHeight || 1)) * 100}
                            r="0.9"
